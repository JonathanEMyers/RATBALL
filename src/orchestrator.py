...
i.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # do not wait so long after program terminates to close socket
i.connect((ingest_host_IP, ingest_listener_port)) # client side should connect, server should bind
logger.info("Connected to ingestor server.")
logger.info(f"Listening on {ingest_host_IP}:{ingest_listener_port}...")

b = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
b.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # So the system does not wait so long after the program terminates to close the socket
b.connect((BMI_host_IP, BMI_listener_port)) # client side should connect, server should bind
...
def pack_audio_data(audio_metadata, sent_time, audio_data, frame_num):
    return struct.pack(
        ">I3d",
        audio_metadata, unix_time_millis(sent_time), audio_data, frame_num,
    )
